...
if(isset($_POST['form1'])) {
    ...
    if($valid == 1) {
    		...
    		//if there is and add on of the product
    	...
    	if( isset($_FILES['photo']["name"]) && isset($_FILES['photo']["tmp_name"]) )
        {
            ...
            for($i=0;$i<count($photo);$i++)
            {
		        ...
		        if ( $my_ext1=='jpg' || $my_ext1=='png' || $my_ext1=='jpeg' || $my_ext1=='gif' || $my_ext1!='JPG' || $my_ext1!='PNG' || $my_ext1!='JPEG' || $my_ext1!='GIF' ) {
		            $final_name1[$m] = $code.'.'.$my_ext1;
        ...
        //end of the add on of the product
        ...
        if(isset($_POST['size'])) {
			...
			foreach($_POST['size'] as $value) {
				$statement = $pdo->prepare("INSERT INTO tbl_product_size (size_id,p_id,prod_stock) VALUES (?,?,?)");
...
foreach ($result as $row) {
	...
	$pecat_id = $row['pecat_id'];
	$pmcat_id = $row['pmcat_id'];
}
...
if ($pecat_id != 0) {
	...
	$statement = $pdo->prepare("SELECT * 
                        ...
                        WHERE t1.ecat_id=?");
$statement->execute(array($pecat_id));
$result = $statement->fetchAll(PDO::FETCH_ASSOC);
foreach ($result as $row) {
	...
	$statement = $pdo->prepare("SELECT * 
                        ...
                        JOIN tbl_top_category t3
                        ON t1.tcat_id = t3.tcat_id
                        WHERE t1.mcat_id=?");
$statement->execute(array($pmcat_id));
$result = $statement->fetchAll(PDO::FETCH_ASSOC);
foreach ($result as $row) {
    $mcat_id = $row['mcat_id'];
    $tcat_id = $row['tcat_id'];
				...
				<div class="box box-info">
					<div class="box-body">
						...
						<div class="form-group">
							...
							<div class="col-sm-4">
								<select name="mcat_id" class="form-control select2 mid-cat">
		                            ...
		                            <?php
		                            $statement = $pdo->prepare("SELECT * FROM tbl_mid_category WHERE tcat_id = ? ORDER BY mcat_id ASC");
						...
						<div class="form-group">
							...
							<div class="col-sm-4">
								<select name="ecat_id" class="form-control select2 end-cat">
		                            ...
		                            <?php
		                            $statement = $pdo->prepare("SELECT * FROM tbl_end_category WHERE mcat_id = ? ORDER BY ecat_id ASC");
		                            $statement->execute(array($mcat_id));
		                            $result = $statement->fetchAll(PDO::FETCH_ASSOC);   
		                            foreach ($result as $row) {
		                                ?>
		                                <option value="<?php echo $row['ecat_id']; ?>" <?php if($row['ecat_id'] == $pecat_id){echo 'selected';} ?>><?php echo $row['ecat_name']; ?></option>
		                                <?php
		                            }
		                            ?>
		                        </select>
						...
						<div class="form-group">
							...
							<div class="col-sm-8">
								<textarea name="p_short_description" class="form-control" cols="30" rows="10" ><?php echo $p_short_description; ?></textarea>
							</div>
						</div>
						<div class="form-group">
							<label for="" class="col-sm-3 control-label">Features</label>
							<div class="col-sm-8">
								<textarea name="p_feature" class="form-control" cols="30" rows="10" id="editor2"><?php echo $p_feature; ?></textarea>
							</div>
						</div>
						<div class="form-group">
							<label for="" class="col-sm-3 control-label">Conditions</label>
							<div class="col-sm-8">
								<textarea name="p_condition" class="form-control" cols="30" rows="10" id="editor3"><?php echo $p_condition; ?></textarea>
							</div>
						</div>
						<div class="form-group">
							<label for="" class="col-sm-3 control-label">Return Policy</label>
							<div class="col-sm-8">
								<textarea name="p_return_policy" class="form-control" cols="30" rows="10" id="editor4"><?php echo $p_return_policy; ?></textarea>
							</div>
						</div>
						<div class="form-group">
