...
if(isset($_POST['form1'])) {
    ...
    if($path!='') {
        ...
        if( $ext!='mp4' && $ext!='MP4' && $ext!='gif' && $ext!='GIF' && $ext!='mpeg-4' && $ext!='MPEG-4' && $ext!='' && $ext!='MKV') {
            $valid = 0;
            $error_message .= 'You must have to upload video<br>';
        }
    } else {
    	$valid = 0;
        $error_message .= 'You must have to select a video<br>';
    }

	if($valid == 1) {

		$code=rand(0,9874985798798679);
		$final_name = $code.'.'.$ext;
        move_uploaded_file( $path_tmp,'../assets/uploads/videos/'.$final_name );

	
		$statement = $pdo->prepare("INSERT INTO tbl_slidervid (file,heading,content,button,position) VALUES (?,?,?,?,?)");
		$statement->execute(array($final_name,$_POST['heading'],$_POST['content'],$_POST['button_text'],$_POST['position']));
			
		$success_message = 'New Carousel is added successfully!';
				...
				<div class="box box-info">
					<div class="box-body">
						<div class="form-group">
							<label for="" class="col-sm-2 control-label">Video <span>*</span></label>
